 ...
 * with position + collapse persisted per pane id.
 *
 * This is the supported version of what plugins previously had to hand-roll by
 * registering into a chrome slot and rendering `position: fixed` themselves —
 * which silently broke inside any transformed ancestor and had to reimplement
 * drag, clamping, and persistence every time.
 */
...
/** Reserved top chrome (the 34px titlebar) — floating panes never cover it. */
const TITLEBAR = 34

const DEFAULT_SIZE = { width: 240, height: 180 }

type StoredRect = { x: number; y: number; collapsed?: boolean }
...
function FloatingPane({ pane }: { pane: Contribution }) {
  ...
  const anchor = (chrome.anchor ?? 'top-right') as FloatingAnchor
  ...
  return (
    <div
      className={cn(
        'pointer-events-auto fixed z-45 flex flex-col overflow-hidden rounded-lg',
        'border border-(--ui-stroke-secondary) bg-(--ui-bg-elevated) shadow-2xl'
      )}
...
export function FloatingPanes() {
  ...
  const floating = panes.filter(pane => paneChrome(pane).placement === 'floating' && !hidden.has(pane.id))
